...
local _Players = Services.Players
local _ReplicatedStorage = Services.ReplicatedStorage
...
function StopTween(p101)
    pcall(function()
        if not p101 then
            ...
            -- Cancel tween đang chạy trong _tp (mainbloxfriit.lua)
            if getgenv().activeTween and type(getgenv().activeTween) == "userdata" then
...
local Window = Fluent:CreateWindow({
    Title = 'Meyy Hub - Blox Fruit [ Premium ]',
    SubTitle = 'by lgbao_na',
    TabWidth = 135,
...
Toggle:OnChanged(function(p306)
    ...
    if getgenv().TeleportIsland ~= true then
        -- Cancel tween đang chạy ngay lập tức
        if getgenv().activeTween and type(getgenv().activeTween) == "userdata" then
            getgenv().activeTween:Cancel()
            getgenv().activeTween = nil
        end
        ...
        while true do
            ...
            if not getgenv().TeleportIsland then
                -- Cancel tween và thoát loop ngay
                if getgenv().activeTween and type(getgenv().activeTween) == "userdata" then
                    getgenv().activeTween:Cancel()
                    getgenv().activeTween = nil
                end
