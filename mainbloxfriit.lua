...
getgenv().StatusUI = false -- add Status UI and Fps , Pings
...
do
    ...
    while not plr.Character or not plr.Character:FindFirstChild("HumanoidRootPart") do
        Services.ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("SetTeam", "Marines")
        wait()
    end
        ...
        pcall(function()
            if getgenv().OnFarm then
                if block and block.Parent == workspace then
                    ...
                    if getgenv().tpRunning then
                        -- Đang tween: chỉ sync block theo player, không snap player về block
                        if b then block.CFrame = b.CFrame end
                    elseif b and (b.Position - block.Position).Magnitude <= 200 then
...
function _tp(aL)
    pcall(function()
        ...
        if player and character and humanoid and rootPart and humanoid.Health > 0 then
            ...
            if getgenv().currentTween and type(getgenv().currentTween) == "userdata" then
            ...
            local DefaultY = aL.Position.Y
            local TargetY = aL.Position.Y
            local targetCFrameWithDefaultY = CFrame.new(aL.Position.X, DefaultY, aL.Position.Z)
            ...
            getgenv().tpRunning = true
            local rs = game:GetService("RunService")
            local blockSyncConn = rs.Heartbeat:Connect(function()
                if block and block.Parent == workspace then
                    block.CFrame = rootPart.CFrame
                end
            end)
            local tweenService = game:GetService("TweenService")
            local b1 = CFrame.new(rootPart.Position.X, DefaultY, rootPart.Position.Z)
            local IgnoreY = true
            local IgnoreYCheck = (b1.Position - targetCFrameWithDefaultY.Position).Magnitude
            if IgnoreY and IgnoreYCheck > 5 then
                rootPart.CFrame = CFrame.new(rootPart.Position.X, DefaultY, rootPart.Position.Z)
                local tweenDuration = (targetPos - rootPart.Position).Magnitude / TweenSpeed
                local tweenInfo = TweenInfo.new(tweenDuration, Enum.EasingStyle.Linear)
                getgenv().activeTween = tweenService:Create(rootPart, tweenInfo, { CFrame = targetCFrameWithDefaultY })
                getgenv().activeTween:Play()
                getgenv().activeTween.Completed:Wait()
                rootPart.CFrame = CFrame.new(rootPart.Position.X, TargetY, rootPart.Position.Z)
            else
                local tweenDuration = (targetPos - rootPart.Position).Magnitude / TweenSpeed
                local tweenInfo = TweenInfo.new(tweenDuration, Enum.EasingStyle.Linear)
                getgenv().activeTween = tweenService:Create(rootPart, tweenInfo, { CFrame = aL })
                getgenv().activeTween:Play()
                getgenv().activeTween.Completed:Wait()
                rootPart.CFrame = CFrame.new(rootPart.Position.X, TargetY, rootPart.Position.Z)
            end
            blockSyncConn:Disconnect()
            getgenv().tpRunning = false
    ...
    while wait(1) do
        if _G.Auto_Cake_Prince then
            pcall(function()
                ...
                -- Khai báo bigMirror ở ngoài để dùng được ở cả 2 nhánh
                local bigMirror = nil
                local bigMirrorOpen = false

                if not _G.IgnoreKata then
                    local cakeLoaf = workspace.Map:FindFirstChild("CakeLoaf")
                    if not cakeLoaf then _tp(CFrame.new(-2077, 252, -12373)) return end

                    bigMirror = cakeLoaf:FindFirstChild("BigMirror")
                    if not bigMirror then _tp(CFrame.new(-2077, 252, -12373)) return end

                    if not bigMirror:FindFirstChild("Other") then _tp(CFrame.new(-2077, 252, -12373)) return end

                    bigMirrorOpen = bigMirror.Other.Transparency == 0

                    if bigMirrorOpen or Enemies:FindFirstChild("Cake Prince") then
                        ...
                        if v then
                            ...
                            if bigMirrorOpen and (CFrame.new(-1990.67, 4533, -14973.67).Position - Root.Position).Magnitude >= 2000 then
                ...
                if v then
                    ...
                    repeat
                        task.wait()
                        local Killed = string.match(replicated.Remotes.CommF_:InvokeServer("CakePrinceSpawner"), "%d+")
                        _G.StatusItemLabel.Text = "Farm mob to Attack Katakuri " .. (Killed or "0")
                        _G.StatusMobLabel.Text = "Attack Mob : " .. v.Name
                        Attack.Kill(v, _G.Auto_Cake_Prince)

                        -- Refresh bigMirrorOpen trong loop nếu đang check
                        if not _G.IgnoreKata and bigMirror and bigMirror:FindFirstChild("Other") then
                            bigMirrorOpen = bigMirror.Other.Transparency == 0
                        end
                    until not _G.Auto_Cake_Prince
                        or not v.Parent
                        or v.Humanoid.Health <= 0
                        or (not _G.IgnoreKata and bigMirrorOpen)
    ...
    while wait(Sec) do
        if _G.Dojoo then
            pcall(function()
                ...
                local progress = game:GetService("ReplicatedStorage")
