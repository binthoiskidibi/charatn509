...
FarmSubmergedIsland = Tabs.Main:AddToggle("FarmSubmergedIsland", {
    Title = "Farm Submerged Island ( If Level > 2600 ) ",
    Description = "",
    Default = false
})

FarmSubmergedIsland:OnChanged(function(Value)
    _G.FarmSubmergedIsland = Value
end)

spawn(function()
    while wait(1) do
        if _G.FarmSubmergedIsland then
            if Lv >= 2600 then
                local args = {
                    [1] = "TravelToSubmergedIsland"
                }
 
                replicated.Modules.Net:WaitForChild("RF/SubmarineWorkerSpeak"):InvokeServer(unpack(args))
